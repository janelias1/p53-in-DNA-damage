...
% Simple p53-Mdm2 negative feedback + undetermined positive feedback
...
dy = 1;
m = 2;
g_param = [ks, k3, K3, k1, K1, dx, n, k2, K2, dy, m];
...
global g_param;
% ks, k3, K3, k1, K1, dx, n, k2, K2, dy, m
...
% equations
% X = [p53]
du(1)=ks + (k3*u(1).^m)./(K3^m+u(1).^m) - k1*u(2)*u(1)./(K1+u(1)) - dx*u(1);
% y = [Mdm2]
...
axes('FontSize',22);
ax = subplot(1,1,1); hold on;
plot(ax,x1,(ks + (k3*x1.^m)./(K3^m + x1.^m) - dx*x1).*((K1+x1)./(k1*x1)),'LineWidth',3,'Color',newcolors(1,:),'DisplayName','p53-nullcline');
plot(ax,x1,(k2/dy)*(x1.^n)./(K2^n+x1.^n),'LineWidth',3,'Color',newcolors(2,:),'DisplayName','Mdm2-nullcline');
plot(ax,x2,(ks + (k3*x2.^m)./(K3^m + x2.^m) - dx*x2).*((K1+x2)./(k1*x2)),'LineWidth',3,'Color',newcolors(1,:),'HandleVisibility','off');
...
initdata = [X0(1),Y0(1)];
[tsol,sol] = ode23(@ODE_01,timeint1*3,initdata,options);
plot(sol(8000:end,1),sol(8000:end,2),'color','k','LineWidth',3);
axis([0 L 0 L]);
box on;
xlabel('p53','FontSize',24,'interpreter','latex'); 
ylabel('Mdm2','FontSize',24,'interpreter','latex');
